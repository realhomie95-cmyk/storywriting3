#!/usr/bin/env node
/**
 * Assemble batch JSON into TXT package files. Zero agent tokens.
...
const packageLines = [
  `# ${base.toUpperCase()} | ${ids} | ${batch.conceptSource || ''}`,
  '',
];
...
fs.writeFileSync(photosPath, batch.items.map(photoBlock).join('\n\n---\n\n'), 'utf8');
